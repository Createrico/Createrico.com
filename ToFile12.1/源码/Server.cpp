...
void agt(string mode, string color)
{
	...
	else if (color == "GB") SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), FOREGROUND_INTENSITY | FOREGROUND_GREEN | FOREGROUND_BLUE);
	cout << "● ";
	...
	while (1)
	{
		...
		while (1)
		{
			if (Mode[0] == 'A')
			{
				...
				if (zt[0] == 'F')
				{
					agt("ahms", "GB"); cout << "Start to received the folder" << endl;
					continue;
				}
				else if (zt[0] == 'E')
				{
					agt("ahms", "R"); cout << "Folder is empty" << endl;
					continue;
				}
				waitY(4);
				...
				agt("ahms", "GB"); cout << "file size: " << lrecvlong << endl;
				waitM(7);
			    while (1)
				{
				...
				agt("ahms", "GB"); cout << "File received and saved as " << Fp << endl;
				waitM(8);
			}
			if (Mode[0] == 'B')
			{
				...
				else
				{
					...
					else send(client, "Y", 1, 0), agt("ahms", "GB"), cout << "Find code file: " << findname << endl;
			    ...
			    agt("ahms", "GB"); cout << "File sent" << endl;
			    fclose(file);
			    waitY(9);
			}
		}
